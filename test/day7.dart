...
class BagRules {
  ...
  BagRules(List<BagRule> rules)
      : rules = {for (var rule in rules) rule.color: rule};

  static BagRules parse(String input) =>
      BagRules(LineSplitter.split(input).map(BagRule.parse).toList());

  bool canContain(BagRule rule, String color) {
    return rule.constraintsByColor.containsKey(color) ||
        rule.constraints.any((c) => canContain(rules[c.color]!, color));
  }

  final _totalBagsCache = <String, int>{};
  int totalBags(BagRule rule) => _totalBagsCache.putIfAbsent(
      rule.color,
      () => rule.constraints.fold(
          1 /* bag itself */,
          (count, constraint) =>
              count + constraint.number * totalBags(rules[constraint.color]!)));
}

class BagRule {
  final String color;
  final List<Constraint> constraints;
  final Map<String, Constraint> constraintsByColor;
  BagRule(this.color, this.constraints)
      : constraintsByColor = {
          for (var constraint in constraints) constraint.color: constraint
        };

  static BagRule parse(String input) {
    var parts = input.split(' bags contain ');
    var color = parts.first;
    var constraintPart = parts.last.trim();
    if (constraintPart.endsWith('.')) {
      constraintPart = constraintPart.substring(0, constraintPart.length - 1);
    }

    if (constraintPart == 'no other bags') {
      return BagRule(color, <Constraint>[]);
    }
    return BagRule(
        color, constraintPart.split(',').map(Constraint.parse).toList());
  }
}
...
class Constraint {
  ...
  const Constraint(this.number, this.color);

  static Constraint parse(String input) {
    var match = constraintPattern.firstMatch(input.trim())!;
    return Constraint(int.parse(match.group(1)!), match.group(2)!);
  }
}

final constraintPattern = RegExp(r'^(\d+) (\w+ \w+) bags?');
...
int part1(String input) {
  var rules = BagRules.parse(input);
  var color = 'shiny gold';
  return rules.rules.values
      .where((rule) => rule.color != color && rules.canContain(rule, color))
      .length;
}
...
int part2(String input) {
  var rules = BagRules.parse(input);
  var color = 'shiny gold';
  return rules.totalBags(rules.rules[color]!) - 1;
}

void main() {
  test('sample', () {
    expect(part1(sampleInput), 4);
    expect(part2('''shiny gold bags contain 2 dark red bags.
