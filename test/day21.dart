...
class Allergen {
  ...
  @override
  String toString() => 'Allergen($name)';

  @override
  int get hashCode => name.hashCode;
  @override
  bool operator ==(Object other) => other is Allergen && other.name == name;
}
...
class Ingredient {
  ...
  int get hashCode => name.hashCode;
  @override
  bool operator ==(Object other) => other is Ingredient && other.name == name;
}
...
class Product {
  ...
  Product(this.ingredients, this.allergens);

  String toString() =>
      '${ingredients.map((i) => i.name).join(' ')} (contains ${allergens.map((a) => a.name).join(',')})';
  static Product parse(String input) {
    var parts = input.split('(contains ');
    var ingredients =
        parts.first.trim().split(' ').map((i) => Ingredient(i)).toSet();
    var apart = parts.last.substring(0, parts.last.length - 1);
    var allergens = apart.split(', ').map((a) => Allergen(a)).toSet();
...
class Products {
  ...
  static Products parse(String input) =>
      Products(LineSplitter.split(input).map(Product.parse).toList());
}

int part1(String input) {
  var p = Products.parse(input);
  var allAllergens = p.products.expand((p) => p.allergens).toSet();
  var allIngredients = p.products.expand((p) => p.ingredients).toSet();

  var candidates = <Allergen, Set<Ingredient>>{};
  // Initially, everything can contain everything.
  for (var allergen in allAllergens) {
    candidates[allergen] = Set.of(allIngredients);
  }

  for (var product in p.products) {
    for (var allergen in product.allergens) {
      // If allergen listed, it must be in ingredients.
      candidates[allergen] =
          candidates[allergen]!.intersection(product.ingredients);
    }
  }
  ...
  while (true) {
    var progress = false;
    for (var allergen in allAllergens) {
      if (candidates[allergen]!.length == 1) {
        var ingredient = candidates[allergen]!.single;
        for (var a in allAllergens) {
          if (a == allergen) {
            continue;
          }
          progress |= candidates[a]!.remove(ingredient);
        }
      }
    }
    if (!progress) {
  ...
  var safeIngredients =
      allIngredients.difference(candidates.values.expand((v) => v).toSet());

  var result = 0;
  for (var product in p.products) {
    for (var ingredient in safeIngredients) {
...
String part2(String input) {
  ...
  return (candidates.entries.where((e) => e.value.length == 1).toList()
        ..sort((a, b) => a.key.name.compareTo(b.key.name)))
      .map((e) => e.value.single.name)
      .toList()
      .join(',');
}
...
void main() {
  test('parse', () {
    var p = Product.parse('sqjhc mxmxvkd sbzzf (contains fish)');
    expect(p.allergens.single.name, 'fish');
    expect(p.ingredients.map((i) => i.name), {'sqjhc', 'mxmxvkd', 'sbzzf'});
    expect(Products.parse(input).products.length, 40);
  });
