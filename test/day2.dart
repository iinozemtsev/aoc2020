...
class Policy {
  ...
  Policy(this.char, this.from, this.to);

  bool complies(String password) {
    var count = password.codeUnits.where((c) => c == char).length;
    return (count >= from) && (count <= to);
  }

  bool compliesPart2(String password) {
    var a = password.codeUnitAt(from - 1) == char;
    var b = password.codeUnitAt(to - 1) == char;
    return a ^ b;
  }

  static Policy parse(String policyString) {
    var match = _pattern.firstMatch(policyString)!;
    return Policy(
        match.namedGroup('char')!.codeUnitAt(0),
        int.parse(match.namedGroup('from')!),
        int.parse(match.namedGroup('to')!));
  }
}

final _pattern = RegExp(r'^(?<from>\d+)-(?<to>\d+) (?<char>.)$');

int part1(List<String> input) {
  var result = 0;
  for (var line in input) {
    var parts = line.split(': ');
    var policy = Policy.parse(parts.first);
...
int part2(List<String> input) {
  ...
  for (var line in input) {
    ...
    var policy = Policy.parse(parts.first);
    var password = parts.last;
    if (policy.compliesPart2(password)) {
      result++;
    }
  }
  return result;
...
void main() {
  test('sample', () {
    expect(
        ...
        2);
    expect(Policy.parse('1-3 a').compliesPart2('abcde'), isTrue);
    expect(
        part2([
          '1-3 a: abcde',
          '1-3 b: cdefg',
