...
class Passport {
  ...
  bool isValid() => requiredFields.difference(fields.keys.toSet()).isEmpty;

  bool isVeryValid() {
    // byr (Birth Year) - four digits; at least 1920 and at most 2002.
    if (!isDigitBetween('byr', 1920, 2002)) {
      return false;
    }
    // iyr (Issue Year) - four digits; at least 2010 and at most 2020.
    if (!isDigitBetween('iyr', 2010, 2020)) {
      return false;
    }
    // eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
    if (!isDigitBetween('eyr', 2020, 2030)) {
      return false;
    }
    if (!hgtIsValid()) {
      return false;
    ...
    if (RegExp(r'^#[0-9a-f]{6}$').firstMatch(fields['hcl'] ?? 'invalid') ==
        null) {
      return false;
    }
    ...
    if (RegExp(r'^\d{9}$').firstMatch(fields['pid'] ?? 'invalid') == null) {
      return false;
    }
  ...
  bool hgtIsValid() {
    ...
    var match =
        RegExp(r'^(\d+)(cm|in)').matchAsPrefix(fields['hgt'] ?? 'invalid');
...
// If in, the number must be at least 59 and at most 76.
  ...
  bool isDigitBetween(String field, int from, int to) {
    var value = int.tryParse(fields[field] ?? 'invalid');
    if (value == null) {
      return false;
    }
  ...
  static Passport parse(String input) => Passport({
        for (var kv in input.split(RegExp(r'\s+')).map((kv) => kv.split(':')))
          kv.first: kv.last
      });

  static List<Passport> parseAll(String input) =>
      input.split('\n\n').map(Passport.parse).toList();
}

int part1(String input) =>
    Passport.parseAll(input).where((p) => p.isValid()).length;

int part2(String input) =>
    Passport.parseAll(input).where((p) => p.isVeryValid()).length;

void main() {
  test('sample', () {
    expect(part1('''ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
const input = '''hcl:#6b5442 ecl:brn iyr:2019
